...
window.PYRE2Detail = (()=>{
 class RefinedFields {
  constructor(app,scale){
   ...
   if(this.grid.reduce((a,b)=>a*b,1)>12000000)throw Error('Refinement exceeds the 12-million-cell safety limit. Select a lower velocity tier.');
 ...
 function install(app){
  ...
  app.enableDetail=(scale=0)=>{
   ...
   if(scale&&m.grid.reduce((a,b)=>a*b,1)*scale**3>12000000)throw Error('Refinement exceeds 12 million cells. Reduce the velocity tier first.');
